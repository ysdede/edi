...
class SaleOrder(models.Model):
    ...
    def _ubl_add_quotation_line(
        ...
        line_root = etree.SubElement(parent_node, ns["cac"] + "QuotationLine")
    ...
    def generate_quotation_ubl_xml_etree(self, version="2.1"):
        ...
        self._ubl_add_delivery(self.partner_shipping_id, xml_root, ns, version=version)
    ...
    def generate_order_response_simple_ubl_xml_etree(self, version="2.1"):
        ...
        accepted_indicator = etree.SubElement(xml_root, ns["cbc"] + "AcceptedIndicator")
        accepted_indicator.text = "true"
        order_reference = etree.SubElement(xml_root, ns["cac"] + "OrderReference")
        order_reference_id = etree.SubElement(order_reference, ns["cbc"] + "ID")
    ...
    def generate_ubl_xml_string(self, doc_type, version="2.1"):
        ...
        if doc_type == "quotation":
            xml_root = self.with_context(lang=lang).generate_quotation_ubl_xml_etree(
                version=version
            )
            document = "Quotation"
        elif doc_type == "order":
